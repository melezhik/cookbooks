...
def header 
  ...
  ruby_block 'info' do 
    block do
      ...
      print "local_lib_install has started with rights: user=#{user} group=#{group} "
      print "install-base : #{install_base_print} "
      print "cwd : #{cwd} "
      print "install_version : #{version} "
      print "local_lib_stack : #{local_lib_stack} "
...
def local_lib_stack
  ...
  perl5lib = Array.new
  perl5lib += node.local_lib.default_inc
  perl5lib += @installer.inc

  local_lib_stack = 'eval $(perl -Mlocal::lib=--deactivate-all); '  

  unless  @installer.install_base.nil?
   local_lib_stack << "eval $(perl -Mlocal::lib=#{real_install_base}); "
  end

  local_lib_stack << "PERL5LIB=$PERL5LIB:#{perl5lib.join(':')}; " unless perl5lib.empty?
  return local_lib_stack
