class PsgiSpec < MiniTest::Chef::Spec
  describe 'psgi_application action install' do
    it 'creates proper init script file' do
      ...
      file("/tmp/psgi/default/app").must_include 'MOUNT_OPT=""'
      file("/tmp/psgi/default/app").must_include 'DAEMON_ARGS="-s FCGI $PROCMANAGER_OPT --listen /tmp/app_fcgi.socket -E development $PROCTITLE_OPT $NPROC_OPT $MOUNT_OPT"'
